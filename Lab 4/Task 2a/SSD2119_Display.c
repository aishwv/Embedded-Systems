...
static void LCD_GPIOInit(void){
    ...
    GPIO_PORTD_AFSEL_R &= ~(0x4);               // disable alternate functions
    GPIO_PORTD_DEN_R |= 0x4;                    // enable digital I/O on PD2

    // K5
    SYSCTL_RCGCGPIO_R |= SYSCTL_RCGCGPIO_R9;    // activate port K
    wait++;                                     // wait for port activation
    wait++;                                     // wait for port activation
    GPIO_PORTK_DIR_R |= 0x20;                   // make PK5 an output
    GPIO_PORTK_AFSEL_R &= ~(0x20);              // disable alternate functions
    GPIO_PORTK_DEN_R |= 0x20;                   // enable digital I/O on PK5

    // M7
    SYSCTL_RCGCGPIO_R |= SYSCTL_RCGCGPIO_R11;   // activate port M
    wait++;                                     // wait for port activation
    wait++;                                     // wait for port activation
    GPIO_PORTM_DIR_R |= 0x80;                   // make PM7 an output
    GPIO_PORTM_AFSEL_R &= ~(0x80);              // disable alternate functions
    GPIO_PORTM_DEN_R |= 0x80;                   // enable digital I/O on PM7

    // N4, N5
    SYSCTL_RCGCGPIO_R |= SYSCTL_RCGCGPIO_R12;   // activate port N
    wait++;                                     // wait for port activation
    wait++;                                     // wait for port activation
    GPIO_PORTN_DIR_R |= 0x30;                   // make PN4, PN5 outputs
    GPIO_PORTN_AFSEL_R &= ~(0x30);              // disable alternate functions
    GPIO_PORTN_DEN_R |= 0x30;                   // enable digital I/O on PN4, PN5

    // P0, P1, P3, P4
    SYSCTL_RCGCGPIO_R |= SYSCTL_RCGCGPIO_R13;   // activate port P
    wait++;                                     // wait for port activation
    wait++;                                     // wait for port activation
    GPIO_PORTP_DIR_R |= 0x1B;                   // make PP0-1, PP3-4 outputs
    GPIO_PORTP_AFSEL_R &= ~(0x1B);              // disable alternate functions
    GPIO_PORTP_DEN_R |= 0x1B;                   // enable digital I/O on PP0-1, PP3-4

    // Q0, Q2, Q3
    SYSCTL_RCGCGPIO_R |= SYSCTL_RCGCGPIO_R14;   // activate port Q
    wait++;                                     // wait for port activation
    wait++;                                     // wait for port activation
    GPIO_PORTQ_DIR_R |= 0xD;                    // make PQ0, PQ2-3 outputs
    GPIO_PORTQ_AFSEL_R &= ~(0xD);               // disable alternate functions
    GPIO_PORTQ_DEN_R |= 0xD;                    // enable digital I/O on PQ0, PQ2-3
}
